...
constexpr std::size_t WORKSPACESIZE = 1ULL << 32;
/*
...
struct Params {
    ...
    std::size_t          ws_size  = WORKSPACESIZE;
    precision            prec     = FP32;
    int                  n_view   = 6;  // for multi-view 3D detection
};
...
public:
    ...
    void inference(cv::Mat image);  //推理部分，preprocess-enqueue-postprocess
...
public:
    ...
    virtual bool preprocess_cpu(cv::Mat image)      = 0;
    virtual bool preprocess_gpu(cv::Mat image)      = 0;
