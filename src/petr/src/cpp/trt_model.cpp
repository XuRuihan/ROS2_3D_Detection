...
Model::Model(string onnx_path, logger::Level level, Params params) {
    ...
    m_workspaceSize = params.ws_size;
...
bool Model::build_engine() {
    ...
    if (!parser->parseFromFile(
        ...
        LOGE("Failed to parse ONNX: %s", m_onnxPath.c_str());
        return false;
    }

    if (network->getNbOutputs() <= 0) {
        LOGE("Parsed ONNX network has no outputs: %s", m_onnxPath.c_str());
        return false;
    }
    ...
    const string package_share_dir =
        std::filesystem::path(m_onnxPath).parent_path().parent_path().parent_path().string();
    const string calibration_data_path = package_share_dir + "/calibration/calibration_list.txt";
    const string calibration_table_path = package_share_dir + "/calibration/calibration_table.txt";
    ...
    if (!plan) {
        LOGE("Failed to build serialized TensorRT network for %s", m_onnxPath.c_str());
        return false;
    }
...
*/
void Model::inference(cv::Mat image) {
    if (m_params->dev == CPU) {
        preprocess_cpu(image);
    } else {
